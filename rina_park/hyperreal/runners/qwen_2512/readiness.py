#!/usr/bin/env python3
"""Offline config/load readiness checks for the pinned Qwen-Image-2512 MLX model."""
...
RINA = ROOT / "rina_park"
REGISTRY_PATH = RINA / "models/registry_qwen_image_2512.yml"
MODEL_REPO = "AbstractFramework/qwen-image-2512-8bit"
MODEL_REVISION = "f70648e7146ce762557dcebd40809586c32ec558"
BASE_MODEL_REPO = "Qwen/Qwen-Image-2512"
BASE_MODEL_REVISION = "25468b98e3276ca6700de15c6628e51b7de54a26"
MODEL_PATH = (
    RINA
    / "models/qwen_image_2512"
    / "AbstractFramework-qwen-image-2512-8bit-f70648e"
)
GPU_LOCK_PATH = Path("/tmp/rina-qwen-image-2512-mlx.lock")
OFFLINE_ENV = {
    "HF_HUB_OFFLINE": "1",
...
def load_registry(path: Path = REGISTRY_PATH) -> dict[str, object]:
    # JSON is a strict YAML subset and avoids a runtime dependency in readiness checks.
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload["source"]["repo_id"] != MODEL_REPO:
        raise RuntimeError("registry source repo does not match the pinned runner")
    if payload["source"]["revision"] != MODEL_REVISION:
        raise RuntimeError("registry source revision does not match the pinned runner")
    if payload["base_model"]["repo_id"] != BASE_MODEL_REPO:
        raise RuntimeError("registry base model does not match the pinned runner")
    if payload["base_model"]["revision"] != BASE_MODEL_REVISION:
        raise RuntimeError("registry base revision does not match the pinned runner")
    if payload["license"]["spdx"] != "Apache-2.0":
        raise RuntimeError("model is not registered as Apache-2.0")
    return payload


def verify_artifacts(
    registry: dict[str, object], model_path: Path = MODEL_PATH, verify_hashes: bool = True
) -> dict[str, object]:
    ...
    if set(actual) != set(registered):
        ...
        raise RuntimeError(f"artifact inventory mismatch; missing={missing}, extra={extra}")
    size_bytes = 0
...
def config_readiness(verify_hashes: bool = True) -> dict[str, object]:
    ...
    return {
        ...
        "runner_class": f"{QwenImage.__module__}.{QwenImage.__name__}",
...
def load_readiness(verify_hashes: bool = True) -> dict[str, object]:
    ...
    try:
        with sequential_gpu(), no_network():
            ...
            # MLX parameters are lazy; force materialization so "load_ready" proves
            # that all shards can be read and resident together without inference.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--skip-hashes",
        action="store_true",
        help="Only check inventory and sizes; intended for repeated local diagnostics.",
    )
    return parser
